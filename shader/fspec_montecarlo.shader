...
void main()
{
	...
	for (int i=0;i<sample_count;i++) {
		...
		vec2 hammersley = vec2(float(i)/float(normals),float(vdcorput)*2.3283064365386963e-10);
		// TODO: i really need to work on my bitwise skills

		// lobe geometry cast
		float phi = 2.0*MATH_PI*hammersley.x;
		vec2 theta = vec2(0,sqrt((1.0-hammersley.x)/(1.0+(asq-1.0)*hammersley.y)));
		theta.x = sqrt(1.0-pow(theta.y,2.0));
		vec3 cartesian = vec3(cos(phi)*theta.x,sin(phi)*theta.x,theta.y);
		vec3 lobecast = normalize(ntangent*cartesian.x+nbitangent*cartesian.y+normals*cartesian.z);
		float dlcast = dot(normals,lobecast);
		vec3 light_dir = normalize(2.0*dlcast*lobecast-normals);
		float dlgt_in = max(dot(normals,light_dir),0.0);

		// calculate sample source lod
		float throwbridge_reitz = asq/(MATH_PI*pow(pow(max(dot(normals,lobecast),0.0),2.0)
				* (asq-1.0)+1.0,2.0));
		float pdensity = throwbridge_reitz*max(dlcast,0.0)/(4.0*max(dlcast,0.0))+.0001;
		float map_lod = log2((1.0/(float(sample_count)*pdensity+.0001))/rmaptex)*lodmod;

		// complete sample and add weight
		colours += textureLod(irradiance_map,light_dir,map_lod).rgb*dlgt_in;
		cmb_wangle += dlgt_in;
