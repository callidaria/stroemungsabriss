...
void Game::run(uint32_t &rstate,CCBManager* ccbm)
{
	...
	while (running) {  // ??maybe kill check if flush with static func ref
		...
		else if (m_frame->kb.ka[SDL_SCANCODE_3]&&!trg_deal) crdSystem.set_currency(0,0,1);
		else if (m_frame->kb.ka[SDL_SCANCODE_4]&&!trg_deal) crdSystem.set_currency(1,0,1);
		...
		else if (m_frame->kb.ka[SDL_SCANCODE_7]&&!trg_deal) crdSystem.set_currency(0,0,1,1);
		else if (m_frame->kb.ka[SDL_SCANCODE_8]&&!trg_deal) crdSystem.set_currency(1,0,1,1);
		...
		else if (m_frame->kb.ka[SDL_SCANCODE_N]&&!trg_deal) crdSystem.set_currency(0,1,2,1);
		else if (m_frame->kb.ka[SDL_SCANCODE_M]&&!trg_deal) crdSystem.set_currency(1,1,2,1);
